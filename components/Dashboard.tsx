...
const Dashboard: React.FC<DashboardProps> = ({ onAddTask, onAddMember }) => {
  ...
  const filteredTasks = useMemo(() => {
    return TASKS.filter(task => getTaskStatus(task.id) === activeTab);
  }, [activeTab]);

  // Calculate counts dynamically
  const getCount = (status: TabStatus) => TASKS.filter(t => getTaskStatus(t.id) === status).length;
    ...
    <main className="flex-1 p-8 h-full overflow-y-auto custom-scrollbar">
      ...
      <div className="flex flex-col gap-1 mb-6">
        <div className="flex items-center gap-2 text-xs text-gray-400 dark:text-gray-500 font-medium mb-1">
          ...
          <span>&gt;</span>
          <span className="text-gray-800 dark:text-gray-200 font-bold">Taskly - Saas Dashboard</span>
        </div>
        
        <div className="flex items-end justify-between">
          <div className="flex items-center gap-4">
             <h1 className="text-3xl font-bold text-gray-900 dark:text-white tracking-tight">Taskly - Saas Dashboard</h1>
          </div>
